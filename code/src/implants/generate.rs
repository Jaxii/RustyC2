...
) -> bool
{
    ...
    match implant_project_path.as_path().to_str() {
        ...
        Some(implant_project_path_str) =>
        {
            ...
            .env("ImplantCommandWhoami", CONFIG.implant.tasks.commands[0].code.to_string())
            .env("ImplantCommandPwd", CONFIG.implant.tasks.commands[1].code.to_string())
...
) -> bool
{
    ...
    println!("[+] Generating executable for HTTP listener on port {}", http_listener.port);
...
pub fn list_http_implants(
    output_implant_projects: &mut Vec<String>
) -> bool
{
